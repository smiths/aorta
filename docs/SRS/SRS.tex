...
\begin{figure}[hbpt!]
\centering
\includegraphics[width=100mm]{humanOrgans}
...
% \hline
  ...
  \hline
  Description & 
                A visual representation of something that is represented using only two spatial dimensions with a sequence of arrays where a voxel (\ddref{v}) represents the color or intensity. Each move in the transverse plane (Figure \ref{ACS}) is considered as one slice
  \\
  \hline
  Sources & \cite{Nejad2017} \\
  \hline
...
$ j \in [0..n-1] \wedge $
$ k \in[0..p-1] :$\\
$ (((i-Seed\_a_k[0])^2 + (j-Seed\_a_k[1])^2  \leq radius) \text{ } \lor $\\
$ ((i-Seed\_d_k[0])^2 + (j-Seed\_d_k[1]^2 \leq radius)) $ \\
$ \implies V_\text{in}[i,j,k] \in \text{structure} )$ \\
...
\begin{figure}[H]
\centering
\includegraphics[width=110mm]{Aorta_seeds}
...
\item[NFR\refstepcounter{nfrnum}\thenfrnum \label{NFR_Accuracy}:] \textbf{Consistency}
