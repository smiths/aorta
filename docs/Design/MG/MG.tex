...
\item [\refstepcounter{mnum} \mthemnum \label{mSAS}:] Sagittal Aorta Segmentation Module
\item ...
\end{description}
...
\item[Use case:] This module should always have a \emph{begin\_segmentation} function available for the developer to begin the aorta segmentation.
\end{description}
...
Call the method \emph{prepared\_image} from DAS object to crop the image into desired size.
...
\subsubsection{Sagittal Aorta Segmentation Module (\mref{mSAS})}
\begin{description}
\item[Secrets:] The algorithm to segment the Aorta from sagittal plane, this can segment out the curve part of the Aorta.
\item[Services:] Includes data structure and algorithms used in the system that
  do not provide direct interaction with the user. 
...
\item[Implemented By:] QT/Tkinter
\end{description}
