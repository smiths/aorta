...
\multirow{3}{0.3\textwidth}{Software Decision Module} & Aorta Segmentation Module\\
& Descending Aorta Segmentation Module\\
& Ascending Aorta Segmentation Module\\
& Aorta Sagital Segmentation Module\\
\bottomrule
...
\subsubsection{Aorta Segmentation Module (\mref{mAS})}
\begin{description}
\item[Secrets:] The abstract class for an Aorta segmenter.
\item[Services:] Includes the common attributes and function signatures of any Aorta segmenter.
\item[Use case:] When declare a new segmenter class, AS is used as the base class for reference to know what an Aorta segmenter must have.
 It must have an original image, a segmented image and a \emph{begin\_segmentation} method public to the control module to start the segmentation. 
\end{description}
