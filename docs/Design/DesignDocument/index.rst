...
At the beginning of the algorithm, the user inputs the integer coordinates indicating the position of the descending aorta or ascending aorta centre.

.. note::
   The algorithm works separately on the descending aorta segmentation and the ascending aorta segmentation. Thus, it will ask users to input the coordinate of the descending aorta centre. After completing this segmentation, it will prompt the user for the input variable of the ascending aorta.

The user's chosen slice will be used as a reference throughout the workflow of the algorithm. Therefore, changing the input of the centre coordinate and different slices could generate a slightly different segmentation result
...
.. code-block:: python
   ...
   for slice_i in range(start, end, step):
       ...
       # element-wise operation, exactly what we doing with the double for-loop to assign white pixel and black pixel
