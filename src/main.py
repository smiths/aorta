...
def get_images(path, num_slices):
    ...
    # If you do not manually enter these values, 
...
if __name__ == '__main__':
    ...
    desc_axial_segmenter = AortaSegmenter.AortaDescendingAxialSegmenter(startingSlice=86,
        aortaCentre=[112, 151], numSliceSkipping=3, segmentationFactor=2.2,
        segmentingImage=image)

    desc_axial_segmenter.prepared_segmenting_image(image=image, index=(190, 165, 40), size=(191, 216, 335))
    ...
    asc_axial_segmenter = AortaSegmenter.AortaAscendingAxialSegmenter(startingSlice=114,
        aortaCentre=[40, 40], numSliceSkipping=3, segmentationFactor=2.2,
        segmentingImage=cropped_image, segmentedImage=processed_image)
    ...
    sag_segmenter = AortaSegmenter.AortaSagitalSegmenter(segmentationFactor=3.5,
        segmentedImage=processed_image, original_cropped_image=cropped_image)

    sag_segmenter.begin_segmentation()

    writer = sitk.ImageFileWriter()
    writer.SetImageIO("VTKImageIO")
    outFilePath = "../result/result_{datetime}_.vtk".format(datetime=datetime.now().strftime("%Y-%m-%d_%H%M"))
    writer.SetFileName(outFilePath)
