...
class AortaAscendingAxialSegmenter(AortaAxialSegmenter):
    ...
    def __init__(self, startingSlice, aortaCentre, numSliceSkipping, segmentationFactor, 
                 segmentingImage, segmentedImage, normalized = False, outputBinary = True):
        self._segmented_image = segmentedImage

        super().__init__(startingSlice, aortaCentre, numSliceSkipping, 
            segmentationFactor, segmentingImage, normalized, outputBinary)
    ...
    def __top_to_bottom_segmentation(self):
        fully_seg_slice, self._original_size, centre_previous, _, _, self._seeds = self.__circle_filter(
            self._starting_slice, self._aorta_centre, [])

        previous_size = self._original_size
        seeds_previous = self._seeds

        # assign both the circle from the ascending aorta and descending aorta to
        # the current slice
        self._segmented_image[:, :, self._starting_slice] = (
            fully_seg_slice > 0) | self._segmented_image[:, :, self._starting_slice]

        # counts how many slices have been skipped
        counter = 0
        num_skips = 3
        more_circles = True
        # goes from current slice to the bottom of the ascending aorta (opposite
        # direction from the arch)
        for sliceNum in range(self._starting_slice + 1, self._segmenting_image.GetDepth()):
            ...
            if (more_circles):
                ...
                # determine whether the size of this slice "qualifies" it to be
                # accurate
                ...
                is_new_center_qualified = is_new_center_qualified \
                    and (total_coord < self._segmentation_factor * self._original_size) \
                    and (total_coord < 2 * previous_size)

                if is_new_center_qualified:
                    
                    counter = 0
                    self._segmented_image[:, :, sliceNum] = (seg > 0) | self._segmented_image[:, :, sliceNum]
                    centre_previous = centre
                    seeds_previous = seeds

                # otherwise skip slice and don't change previous centre and seed
                # values
                else:
                    counter += 1
                    if (counter >= num_skips):
                        ...
                        if not self._is_output_binary:
                            self._segmented_image[:, :, i] = sitk.Cast(
                                self._segmenting_image_255[:, :, i], sitk.sitkVectorUInt8)
    ...
    def __bottom_to_top_segmentation(self):
        ...
        # factor size that starts like normal, but may change depending on overlap
        factor_size_overlap = self._segmentation_factor

        # faulty slice -- 0 can mean good, 1 can mean didn't reach double size in
        # arch
        faulty = 1
        num_skips = 3

        # when size starts decreasing after it has enlarged to double the normal
        # size, mark True
        decreasing_size = False

        # goes from current slice to the top of the ascending aorta (towards the
        # arch)
        for sliceNum in reversed(range(0, self._starting_slice)):
            if (more_circles):
                ...
                if is_new_center_qualified:
                    ...
                    if not self._is_output_binary:
                        self._segmented_image[:, :, sliceNum] = sitk.Cast(
                            self._segmenting_image[:, :, sliceNum], sitk.sitkVectorUInt8)
                    if (counter >= num_skips):
                        more_circles = False
                ...
                self._segmented_image[:, :, sliceNum] = sitk.Cast(
                    self._segmenting_image_255[ct_number][:, :, sliceNum], sitk.sitkVectorUInt8)
    ...
    def __circle_filter(self, sliceNum, centre, seeds_previous):
        ...
        ls = self._segment_filter.Execute(init_ls, sitk.Cast(imgSlice, sitk.sitkFloat32))

        # make array from segmentation
        nda = sitk.GetArrayFromImage(ls > 0)

        # Calculate the centre of the segmentation by 1st getting the average y value. Then, finding the average x value ON that y-value to ensure there is an actual point there
        # Also determine 4 seed values by going 25% outwards in the x, -x, y and
        # -y directions.
        centre_new = [0, 0]

        # calculate the average y value of the pixels and the height (Δy) of the
        ...
        # calculate the average x value of the pixels along the predetermined y
        # value. also calulate the width (Δx) along that height
        list_x = np.where(nda[centre_new[1]] == 1)[0]
        width = len(list_x)

        # if there were no pixels along that y value for whatever reason, just get
        # the centre of gravity of the segmentation by finding the overall average
        # x value
        if (width == 0):
            _, list_x = np.where(nda == 1)
        centre_new[0] = int(np.average(list_x))

        # determine next seed values; y1,x1, y2,x2 are vertical values; y3,x3, y4,x4 are horizontal
        # the seed values are 25% outwards from the centre. (25% refers to 25% of
        # the entire width or height of the segmentation)
        seeds = []
        ...
        return ls, total_coord, centre_new, lower_threshold, upper_threshold, seeds